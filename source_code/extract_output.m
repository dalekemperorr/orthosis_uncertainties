...
%gamma combination
function [T] = extract_output( Delta_Alpha, Delta_Beta, Delta_Gamma, Delta_T, GammaValues_vector )
...
for(i=1:m)
    T(i, 1)= max(Delta_Alpha(i,:));
    T(i, 2)= max(Delta_Beta(i,:));
    T(i, 3)= max(Delta_Gamma(i,:));
    T(i, 4)= max(Delta_T(i,:));
